...
重新上传并建立知识库会整体替换旧知识库，同时清除全部历史会话，避免旧回答与新知识库混用。清空知识库也会清除全部历史会话。
...
当前共有 25 项自动化测试。流式测试覆盖结构化分片、跨分片 JSON 转义、Ollama 流中错误、缺少结束标记、会话隔离以及失败不写入历史；路由测试覆盖年假快速路由、连续问题上下文继承和证据复查问题复述保护。
